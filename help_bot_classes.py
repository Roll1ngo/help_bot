...
class ShowInfo(ABC):
    def show_info(self, output_date):
        ...


class RequestShowTable(ShowInfo):
    def show_info(self, outpute_date) -> str:
        return outpute_date


class RequestShowString(ShowInfo):
    def show_info(self, outpute_date) -> str:
        return outpute_date.show_all_records()


class RequestShowHelp(ShowInfo):
    def show_info(self, outpute_date: dict) -> str:
        result = ""
        for key, value in outpute_date.items():
            result += f"{key:22}:   {value}\n"

        return result


class Handler:
    def __init__(self, request: ShowInfo):
        self.request = request

    def output_date(self, outpute_date):
        return f"{self.request.show_info(outpute_date)}"
...
class AddressBook(UserDict):
    ...
    def show_all_records(self):
        ...
        return f"{rec.name}||{phone}||{rec.email}||{rec.address}||{rec.birthday}"
